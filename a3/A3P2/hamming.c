...
static int count_ones(const int i);
static int nth_power_base_2(const int n);
static int highest_nth_power(const int i, const int acc);


// hamming(a,b) Returns the number of bits
//   in which a and b differ.
// requires: a >= 0, b >= 0
int hamming(int a, int b) {
    assert((a >= 0) && (b >= 0));
    const int a_bits = count_ones(a);
    const int b_bits = count_ones(b);
    const int diff_bits = a_bits - b_bits;
